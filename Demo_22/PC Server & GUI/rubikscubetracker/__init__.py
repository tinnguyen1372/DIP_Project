        ...
        while True:
            ...
            if self.save_colors and self.size and len(self.data.keys()) == (self.size * self.size):
                ...
                if self.name == 'F':
                    ...
                    # cube.enter_scan_data(jsonself.total_data)
                    cube.crunch_colors()
                    cube.print_cube()
                    output = "".join(cube.cube_for_kociemba_strict())
                    print(output)
                    solution = twophase.solve(output,19,2)
                    solution = solution[:solution.strip().rfind(" ")]
                    print(solution)
                    cv2.putText(self.image, "Calculating solution", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1)
